        ...
        <div>
            ...
            <div className="grid-3" style={{ alignItems: 'start' }}>
                {groupedDebts.map(group => {
                    const isFullyPaid = group.total_owed <= group.total_paid;
                    const progress = (group.total_paid / group.total_owed) * 100 || 0;
                            ...
                            >
                                ...
                                <div style={{ background: 'rgba(0,0,0,0.2)', padding: 'var(--spacing-md)', borderRadius: 'var(--radius-md)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--spacing-xs)' }}>
                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Owed: Rs {group.total_owed.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                        <span style={{ color: 'var(--status-success)', fontSize: '0.875rem', fontWeight: 'bold' }}>Paid: Rs {group.total_paid.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                    </div>
                                ...
                                {!isFullyPaid && (
                                    <div style={{ padding: 'var(--spacing-md)', background: 'var(--glass-bg)', borderTop: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        Remaining Balance: Rs {(group.total_owed - group.total_paid).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                    </div>
                                )}
                            </div>
                                        ...
                                        <div key={debt._id} className="glass-panel" style={{ padding: 'var(--spacing-md)', display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'var(--spacing-xs)', borderTop: '1px solid var(--glass-border)', paddingTop: 'var(--spacing-sm)' }}>
                                                <div style={{ fontWeight: '600', display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                                                    <span style={{ color: 'var(--text-secondary)' }}>Rs {debt.amount_paid.toLocaleString()}</span>
                                                    <span style={{ color: 'var(--text-tertiary)' }}> / Rs {debt.amount_owed.toLocaleString()}</span>
                                                </div>
                                                ...
                                                <div style={{ display: 'flex', gap: 'var(--spacing-sm)' }}>
                                                    {debt.status !== 'paid' && (
                                                        <button className="btn btn-secondary" style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem' }}
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                setPayment({ ...payment, amount: debt.amount_owed - debt.amount_paid });
            ...
            {showPayModal && (
                <div className="modal-overlay" onClick={() => setShowPayModal(null)}>
                    <div className="glass-panel modal-content" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2 className="modal-title">Payment from {showPayModal.friend_name}</h2>
                        ...
                        <div style={{ padding: 'var(--spacing-sm)', background: 'var(--glass-bg)', borderRadius: 'var(--radius-md)', marginBottom: 'var(--spacing-lg)', textAlign: 'center', color: 'var(--text-secondary)' }}>
                            Remaining balance owed: Rs {(showPayModal.amount_owed - showPayModal.amount_paid).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </div>
                        ...
                        <form onSubmit={handlePayDebt}>
                            <div className="grid-2">
                                <div className="form-group">
                                    ...
                                    <input type="number" step="0.01" max={showPayModal.amount_owed - showPayModal.amount_paid} className="form-control" required
                            ...
                            <div className="form-group">
                                ...
                                <small style={{ color: 'var(--text-tertiary)', marginTop: '4px', display: 'block' }}>This will automatically create an income transaction.</small>
                            </div>
